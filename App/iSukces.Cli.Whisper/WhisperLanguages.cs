// ReSharper disable All
namespace iSukces.Cli.Whisper;

/// <summary>
/// Language codes and names accepted by OpenAI Whisper CLI.
/// </summary>
public static class WhisperLanguages
{
    public const string Af = "af";

    public const string Am = "am";

    public const string Ar = "ar";

    public const string As = "as";

    public const string Az = "az";

    public const string Ba = "ba";

    public const string Be = "be";

    public const string Bg = "bg";

    public const string Bn = "bn";

    public const string Bo = "bo";

    public const string Br = "br";

    public const string Bs = "bs";

    public const string Ca = "ca";

    public const string Cs = "cs";

    public const string Cy = "cy";

    public const string Da = "da";

    public const string De = "de";

    public const string El = "el";

    public const string En = "en";

    public const string Es = "es";

    public const string Et = "et";

    public const string Eu = "eu";

    public const string Fa = "fa";

    public const string Fi = "fi";

    public const string Fo = "fo";

    public const string Fr = "fr";

    public const string Gl = "gl";

    public const string Gu = "gu";

    public const string Ha = "ha";

    public const string Haw = "haw";

    public const string He = "he";

    public const string Hi = "hi";

    public const string Hr = "hr";

    public const string Ht = "ht";

    public const string Hu = "hu";

    public const string Hy = "hy";

    public const string Id = "id";

    public const string Is = "is";

    public const string It = "it";

    public const string Ja = "ja";

    public const string Jw = "jw";

    public const string Ka = "ka";

    public const string Kk = "kk";

    public const string Km = "km";

    public const string Kn = "kn";

    public const string Ko = "ko";

    public const string La = "la";

    public const string Lb = "lb";

    public const string Ln = "ln";

    public const string Lo = "lo";

    public const string Lt = "lt";

    public const string Lv = "lv";

    public const string Mg = "mg";

    public const string Mi = "mi";

    public const string Mk = "mk";

    public const string Ml = "ml";

    public const string Mn = "mn";

    public const string Mr = "mr";

    public const string Ms = "ms";

    public const string Mt = "mt";

    public const string My = "my";

    public const string Ne = "ne";

    public const string Nl = "nl";

    public const string Nn = "nn";

    public const string No = "no";

    public const string Oc = "oc";

    public const string Pa = "pa";

    public const string Pl = "pl";

    public const string Ps = "ps";

    public const string Pt = "pt";

    public const string Ro = "ro";

    public const string Ru = "ru";

    public const string Sa = "sa";

    public const string Sd = "sd";

    public const string Si = "si";

    public const string Sk = "sk";

    public const string Sl = "sl";

    public const string Sn = "sn";

    public const string So = "so";

    public const string Sq = "sq";

    public const string Sr = "sr";

    public const string Su = "su";

    public const string Sv = "sv";

    public const string Sw = "sw";

    public const string Ta = "ta";

    public const string Te = "te";

    public const string Tg = "tg";

    public const string Th = "th";

    public const string Tk = "tk";

    public const string Tl = "tl";

    public const string Tr = "tr";

    public const string Tt = "tt";

    public const string Uk = "uk";

    public const string Ur = "ur";

    public const string Uz = "uz";

    public const string Vi = "vi";

    public const string Yi = "yi";

    public const string Yo = "yo";

    public const string Yue = "yue";

    public const string Zh = "zh";

    public const string Afrikaans = "Afrikaans";

    public const string Albanian = "Albanian";

    public const string Amharic = "Amharic";

    public const string Arabic = "Arabic";

    public const string Armenian = "Armenian";

    public const string Assamese = "Assamese";

    public const string Azerbaijani = "Azerbaijani";

    public const string Bashkir = "Bashkir";

    public const string Basque = "Basque";

    public const string Belarusian = "Belarusian";

    public const string Bengali = "Bengali";

    public const string Bosnian = "Bosnian";

    public const string Breton = "Breton";

    public const string Bulgarian = "Bulgarian";

    public const string Burmese = "Burmese";

    public const string Cantonese = "Cantonese";

    public const string Castilian = "Castilian";

    public const string Catalan = "Catalan";

    public const string Chinese = "Chinese";

    public const string Croatian = "Croatian";

    public const string Czech = "Czech";

    public const string Danish = "Danish";

    public const string Dutch = "Dutch";

    public const string English = "English";

    public const string Estonian = "Estonian";

    public const string Faroese = "Faroese";

    public const string Finnish = "Finnish";

    public const string Flemish = "Flemish";

    public const string French = "French";

    public const string Galician = "Galician";

    public const string Georgian = "Georgian";

    public const string German = "German";

    public const string Greek = "Greek";

    public const string Gujarati = "Gujarati";

    public const string Haitian = "Haitian";

    public const string HaitianCreole = "Haitian Creole";

    public const string Hausa = "Hausa";

    public const string Hawaiian = "Hawaiian";

    public const string Hebrew = "Hebrew";

    public const string Hindi = "Hindi";

    public const string Hungarian = "Hungarian";

    public const string Icelandic = "Icelandic";

    public const string Indonesian = "Indonesian";

    public const string Italian = "Italian";

    public const string Japanese = "Japanese";

    public const string Javanese = "Javanese";

    public const string Kannada = "Kannada";

    public const string Kazakh = "Kazakh";

    public const string Khmer = "Khmer";

    public const string Korean = "Korean";

    public const string Lao = "Lao";

    public const string Latin = "Latin";

    public const string Latvian = "Latvian";

    public const string Letzeburgesch = "Letzeburgesch";

    public const string Lingala = "Lingala";

    public const string Lithuanian = "Lithuanian";

    public const string Luxembourgish = "Luxembourgish";

    public const string Macedonian = "Macedonian";

    public const string Malagasy = "Malagasy";

    public const string Malay = "Malay";

    public const string Malayalam = "Malayalam";

    public const string Maltese = "Maltese";

    public const string Mandarin = "Mandarin";

    public const string Maori = "Maori";

    public const string Marathi = "Marathi";

    public const string Moldavian = "Moldavian";

    public const string Moldovan = "Moldovan";

    public const string Mongolian = "Mongolian";

    public const string Myanmar = "Myanmar";

    public const string Nepali = "Nepali";

    public const string Norwegian = "Norwegian";

    public const string Nynorsk = "Nynorsk";

    public const string Occitan = "Occitan";

    public const string Panjabi = "Panjabi";

    public const string Pashto = "Pashto";

    public const string Persian = "Persian";

    public const string Polish = "Polish";

    public const string Portuguese = "Portuguese";

    public const string Punjabi = "Punjabi";

    public const string Pushto = "Pushto";

    public const string Romanian = "Romanian";

    public const string Russian = "Russian";

    public const string Sanskrit = "Sanskrit";

    public const string Serbian = "Serbian";

    public const string Shona = "Shona";

    public const string Sindhi = "Sindhi";

    public const string Sinhala = "Sinhala";

    public const string Sinhalese = "Sinhalese";

    public const string Slovak = "Slovak";

    public const string Slovenian = "Slovenian";

    public const string Somali = "Somali";

    public const string Spanish = "Spanish";

    public const string Sundanese = "Sundanese";

    public const string Swahili = "Swahili";

    public const string Swedish = "Swedish";

    public const string Tagalog = "Tagalog";

    public const string Tajik = "Tajik";

    public const string Tamil = "Tamil";

    public const string Tatar = "Tatar";

    public const string Telugu = "Telugu";

    public const string Thai = "Thai";

    public const string Tibetan = "Tibetan";

    public const string Turkish = "Turkish";

    public const string Turkmen = "Turkmen";

    public const string Ukrainian = "Ukrainian";

    public const string Urdu = "Urdu";

    public const string Uzbek = "Uzbek";

    public const string Valencian = "Valencian";

    public const string Vietnamese = "Vietnamese";

    public const string Welsh = "Welsh";

    public const string Yiddish = "Yiddish";

    public const string Yoruba = "Yoruba";

    
}
